...
// Stop charging
void stop_charging() {
  charging = false; // Clear thecharging flag
...
void read_reference() {
  // Set the analog comparator to use the internal 1.1V reference and the capacitor voltage
  ACSR = bit(ACBG) | bit(ACIE); // Set the bandgap reference and enable the interrupt
  ...
  // Wait for the analog comparator interrupt
  while (!(ACSR & bit(ACI))) {} // Wait until the interrupt flag is set
  ACSR |= bit(ACI); // Clear the interrupt flag
  // Calculate the reference voltage
  unsigned long referenceTime = micros(); // Record the reference time
  float referenceTime = (float)(referenceTime - startTime) / 1000000.0; // Calculate the reference time in seconds
  referenceVoltage = INTERNAL_REFERENCE / (1.0 - exp(-referenceTime / (REFERENCE_CAPACITOR * referenceResistance))); // Calculate the reference voltage in millivolts
  // Calculate the supply voltage
  supplyVoltage = referenceVoltage * (referenceResistance + REFERENCE_RESISTOR) / referenceResistance; // Calculate the supply voltage in millivolts
...
// Interrupt service routine for the analog comparator
ISR(ANALOG_COMP_vect) {
  // Do nothing, just set the interrupt flag
}
